...
class QueryProcessor:
    ...
    def process(self, query: str) -> ProcessedQuery:
        ...
        # Expand synonyms for sparse retrieval
        expanded_terms = self._expand_synonyms(keywords)

        q_type, route_params = self._route_query(query)
        route_params["keywords"] = keywords
    ...
    # 路由规则：中文 + 英文关键词 → 问题类型
    _ROUTE_RULES: Dict[str, List[str]] = {
        "summary":  ["总结", "概括", "归纳", "概述", "汇总", "summarize", "overview", "summary"],
        "compare":  ["对比", "比较", "区别", "差异", "哪个更", "哪个好", "vs", "versus",
                     "compare", "comparison", "difference", "better"],
    }

    def _route_query(self, query: str) -> tuple[str, Dict[str, Any]]:
        """Classify query type and return routing hints.

        Returns:
            (route_type, route_params) where route_params includes hints
            like top_k overrides that downstream retrievers can use.
        """
        q_lower = query.lower()
        for route_type, triggers in self._ROUTE_RULES.items():
            for t in triggers:
                if t in q_lower:
                    if route_type == "summary":
                        return ("summary", {"dense_top_k_override": 15, "enable_rerank": False})
                    elif route_type == "compare":
                        return ("compare", {"dense_top_k_override": 10, "sparse_top_k_override": 10})
        return ("factual", {"enable_rerank": True})  # 默认：精确实体查询
    ...
    def _extract_filters(self, query: str) -> tuple[Dict[str, Any], str]:
        ...
        for key, value in matches:
            # Strip surrounding quotes from quoted filter values
    ...
    def _expand_synonyms(self, keywords: List[str]) -> List[str]:
        """Expand keywords with synonyms from synonym map.

        Looks up each keyword (case-insensitive) in the synonym map and
        returns any synonyms not already present in the original keywords.

        Args:
            keywords: Original list of keywords.

        Returns:
            List of synonym terms, deduplicated against original keywords.
        """
        ...
        for kw in keywords:
            synonyms = (
                self.config.synonym_map.get(kw, [])
                or self.config.synonym_map.get(kw.lower(), [])
            )
            for syn in synonyms:
