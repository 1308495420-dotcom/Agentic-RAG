...
class HybridSearchConfig:
    ...
    Attributes:
        ...
        low_confidence_threshold: Score threshold for re-retrieval (0.0 = disabled)
        re_retrieval_top_k: Expanded top-k for second-pass retrieval
    """
    dense_top_k: int = 20
    sparse_top_k: int = 20
    fusion_top_k: int = 10
    ...
    low_confidence_threshold: float = 0.015  # 低于此分数触发二次检索
    re_retrieval_top_k: int = 30
    context_expand_strategy: str = "neighbors"  # Small-to-Big: "neighbors" | "parent_doc" | "none"
    context_expand_window: int = 2  # ±N neighbor chunks to include
...
class HybridSearch:
    ...
    def search(
        ...
        # ── Apply query routing overrides ──────────────────────────
        route_params = processed_query.route_params
        _orig_dense = self.config.dense_top_k
        _orig_sparse = self.config.sparse_top_k
        if "dense_top_k_override" in route_params:
            self.config.dense_top_k = route_params["dense_top_k_override"]
        if "sparse_top_k_override" in route_params:
            self.config.sparse_top_k = route_params["sparse_top_k_override"]
        ...
        # ── Restore config after routing overrides ──────────────────
        self.config.dense_top_k = _orig_dense
        self.config.sparse_top_k = _orig_sparse
        ...
        # Step 5b: Low-confidence re-retrieval
        _re_retrieved = False
        if self.config.low_confidence_threshold > 0 and fused_results:
            top_score = fused_results[0].score if hasattr(fused_results[0], "score") else 0
            if top_score < self.config.low_confidence_threshold:
                logger.info(
                    "Low confidence (top score %.4f < %.4f), re-retrieving...",
                    top_score, self.config.low_confidence_threshold,
                )
                _orig_d = self.config.dense_top_k
                _orig_s = self.config.sparse_top_k
                self.config.dense_top_k = self.config.re_retrieval_top_k
                self.config.sparse_top_k = self.config.re_retrieval_top_k

                re_dense, re_sparse, _, _ = self._run_retrievals(
                    processed_query=processed_query,
                    filters={},  # 二次检索去掉 filter 扩大召回
                    trace=trace,
                )
                self.config.dense_top_k = _orig_d
                self.config.sparse_top_k = _orig_s

                # Merge: keep original results, de-duplicate new ones
                seen_ids = {getattr(r, "chunk_id", "") for r in fused_results}
                for r in (re_dense or []) + (re_sparse or []):
                    rid = getattr(r, "chunk_id", "")
                ...
                _re_retrieved = True
                if trace is not None:
                    trace.record_stage("re_retrieval", {
                        "trigger_score": round(top_score, 4),
                        "threshold": self.config.low_confidence_threshold,
                        "new_results": sum(1 for _ in (re_dense or []) + (re_sparse or [])),
                    })

        # Step 6: Limit to top_k
        final_results = fused_results[:effective_top_k]

        # Step 7: Small-to-Big context expansion (append neighbors AFTER limit)
        if self.config.context_expand_strategy != "none" and final_results:
            try:
                from src.core.query_engine.context_expander import ContextExpander
                vs_col = self.dense_retriever.vector_store.collection
                final_results = ContextExpander().expand(
                    final_results,
                    strategy=self.config.context_expand_strategy,
                    window=self.config.context_expand_window,
                    collection=vs_col,
                )
            except Exception as exc:
                logger.warning("Context expansion failed: %s", exc)
        ...
        if self.config.parallel_retrieval and run_dense and run_sparse:
            ...
            if run_sparse:
                sparse_results, sparse_error = self._run_sparse_retrieval(
                    processed_query.keywords + processed_query.expanded_terms,
                    filters, trace
                )
