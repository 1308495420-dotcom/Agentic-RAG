        ...
        for metric_name in self._metric_names:
            if metric_name == FAITHFULNESS:
                ...
                if embeddings is None:
                    logger.warning("answer_relevancy skipped: no OpenAI-compatible embedding available")
                    continue
    ...
    def _build_wrappers(self) -> tuple:
        ...
        if use_azure_llm:
            ...
            llm_client = AsyncOpenAI(
                ...
                base_url=getattr(llm_cfg, "base_url", None) or "https://api.deepseek.com",
            )
        else:
            raise ValueError(
        ...
        if use_azure_emb:
            ...
            emb_client = AsyncOpenAI(
                ...
                base_url=getattr(emb_cfg, "base_url", None) or "https://api.deepseek.com",
            )
        elif emb_provider == "ollama":
            # Ollama embeddings not supported by Ragas; metrics that need
            # embeddings (answer_relevancy) will be skipped gracefully.
            logger.warning("Ollama embedding not supported by Ragas — answer_relevancy will be skipped")
            return llm, None
        else:
            raise ValueError(
