...
class EvalRunner:
    ...
    def _retrieve(
        ...
        try:
            ...
            # Check query routing: skip rerank for summary-type queries
            pq = result.processed_query if hasattr(result, 'processed_query') else None
            route_rerank = pq.route_params.get("enable_rerank", True) if pq and pq.route_params else True

            # Apply reranking if enabled AND not suppressed by routing
            if has_reranker and results and route_rerank:
