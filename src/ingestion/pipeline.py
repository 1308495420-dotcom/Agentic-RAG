...
class IngestionPipeline:
    ...
    def __init__(
        ...
        # Stage 2: Loader (created on-demand in run() based on file type)
        self.loader: Any = None
        logger.info("  ✓ Loader will be selected at runtime")
    ...
    def run(
        ...
        try:
            ...
            if _suffix in (".md", ".markdown"):
                self.loader = MarkdownLoader()
                logger.info("  Using MarkdownLoader")
            elif _suffix == ".pdf":
                # Read OCR setting from config
                _ingestion = getattr(self.settings, 'ingestion', None)
                _enable_ocr = getattr(_ingestion, 'enable_ocr', False) if _ingestion else False
