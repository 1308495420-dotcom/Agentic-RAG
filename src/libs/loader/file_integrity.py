...
class SQLiteIntegrityChecker(FileIntegrityChecker):
    ...
    def _ensure_database(self) -> None:
        ...
        Automatically migrates old schema (file_hash PRIMARY KEY) to new
        schema (file_hash + collection UNIQUE) to fix the bug where
        re-ingesting the same file into different collections would
        ...
        try:
            ...
            # ── Schema migration ──────────────────────────────────
            # V1 → V2: composite unique key on (file_hash, collection)
            cursor = conn.execute("SELECT sql FROM sqlite_master WHERE type='table' AND name='ingestion_history'")
            existing = cursor.fetchone()
            if existing and "PRIMARY KEY" in existing[0]:
                # Migrate old single-key schema → new composite-key schema
                backup = conn.execute("SELECT * FROM ingestion_history").fetchall()
                ...
                conn.execute("CREATE INDEX IF NOT EXISTS idx_hash ON ingestion_history(file_hash)")
                # Restore old data (NULL collection → 'default')
                for row in backup:
                    collection_val = row[3] if row[3] is not None else 'default'
                    conn.execute(
                        "INSERT OR IGNORE INTO ingestion_history VALUES (?,?,?,?,?,?,?)",
                        (row[0], row[1], row[2], collection_val, row[4], row[5], row[6])
                    )
                conn.commit()
            elif not existing:
                conn.execute("""
                ...
                conn.execute("CREATE INDEX IF NOT EXISTS idx_hash ON ingestion_history(file_hash)")
                conn.commit()
            # If already V2 schema (has UNIQUE not PRIMARY KEY), do nothing
        finally:
            conn.close()
    ...
    def should_skip(self, file_hash: str, collection: Optional[str] = None) -> bool:
        ...
        Only files with status='success' **for the same collection** are
        skipped.  Failed files can be retried.

        Args:
            file_hash: SHA256 hash of the file.
            collection: Optional collection filter.  When *None* checks
                across all collections (backwards-compatible).

        Returns:
            True if file has status='success' for this collection,
            False otherwise.
        """
        collection_val = collection or "default"
        conn = sqlite3.connect(self.db_path)
        try:
            cursor = conn.execute(
                "SELECT status FROM ingestion_history WHERE file_hash = ? AND collection = ?",
                (file_hash, collection_val),
            )
    ...
    def mark_success(
        ...
        Uses (file_hash, collection) as composite key so the same file
        ingested into different collections creates separate records.
        ...
        conn = sqlite3.connect(self.db_path)
        try:
            # Check if (file_hash, collection) pair already exists
            cursor = conn.execute(
            ...
            if result:
                # Update existing record for this specific (file, collection)
                conn.execute("""
                ...
                """, (file_path, now, file_hash, collection_val))
            else:
                # Insert new record
                conn.execute("""
                    ...
                    VALUES (?, ?, 'success', ?, NULL, ?, ?)
                """, (file_hash, file_path, collection_val, now, now))
    ...
    def mark_failed(
        ...
        conn = sqlite3.connect(self.db_path)
        try:
            # Check if (file_hash, collection) pair exists to preserve processed_at
            cursor = conn.execute(
                "SELECT processed_at FROM ingestion_history WHERE file_hash = ? AND collection = ?",
                (file_hash, collection_val)
            )
            ...
            if result:
                ...
                conn.execute("""
                    ...
                    SET file_path = ?,
                        ...
                        updated_at = ?
                    WHERE file_hash = ? AND collection = ?
                """, (file_path, error_msg, now, file_hash, collection_val))
            else:
                # Insert new record
                conn.execute("""
                    INSERT INTO ingestion_history
                    (file_hash, file_path, status, collection, error_msg, processed_at, updated_at)
                    VALUES (?, ?, 'failed', ?, ?, ?, ?)
                """, (file_hash, file_path, collection_val, error_msg, now, now))
    ...
    def remove_record(self, file_hash: str, collection: Optional[str] = None) -> bool:
        """Remove an ingestion record by its file hash and collection.

        Args:
            file_hash: SHA256 hash identifying the record.
            collection: Collection name.  When *None* removes ALL
                records with this hash (across all collections).
